"""
Enhanced Web Search Module with Crawl4AI - Windows Fixed Version
-----------------------------------------------------------------
Features:
- Multiple search engines (Google, Brave, DuckDuckGo)
- Advanced crawling with Crawl4AI (WINDOWS COMPATIBLE)
...
# CRITICAL FIX for Windows: Set proper event loop policy BEFORE imports
if sys.platform == 'win32':
    # Use WindowsSelectorEventLoopPolicy to support subprocess
...
QUALITY_DOMAINS = {
    ...
    'chemicool.com', 'lenntech.com', 'webelements.com', 'rsc.org'
}
...
class SmartCrawler:
    """Advanced web crawling with Crawl4AI - Windows Compatible"""
    ...
    def __init__(self):
        if CRAWL4AI_AVAILABLE:
            self.browser_config = BrowserConfig(
                headless=True,
                verbose=False,
                # Windows-compatible browser args
                extra_args=[
                    "--disable-gpu",
    ...
    async def crawl_url(self, url: str) -> Optional[Dict]:
        ...
        Falls back to basic scraping if Crawl4AI fails
        """
        ...
        try:
            ...
            # Create crawler instance with proper config
            async with AsyncWebCrawler(config=self.browser_config) as crawler:
                ...
                if not result.success:
                    logger.warning(f"⚠️  Crawl failed, falling back to basic scrape: {url}")
                    return await self._basic_scrape(url)
            ...
            logger.warning(f"⚠️  Crawl4AI error for {url}, using fallback: {str(e)[:100]}")
            return await self._basic_scrape(url)
    
    async def _basic_scrape(self, url: str) -> Optional[Dict]:
        """Fallback basic scraping with aiohttp + BeautifulSoup"""
        try:
            headers = {
            ...
            async with aiohttp.ClientSession(timeout=timeout) as session:
                async with session.get(url, headers=headers, allow_redirects=True) as response:
                    ...
                    if not self._is_quality_content(text):
                        logger.warning(f"⚠️  Basic scrape: low quality content from {url}")
                        return None
                    
                    logger.info(f"✅ Basic scrape success {url}: {len(text)} chars")
            ...
            logger.error(f"❌ Basic scrape error for {url}: {str(e)[:100]}")
...
async def process_web_search(query: str, num_results: int = 6) -> Dict:
    ...
    for i, (search_result, crawl_result) in enumerate(zip(search_results, crawled_results)):
        ...
        if isinstance(crawl_result, Exception):
            logger.error(f"Crawl exception for {search_result['url']}: {crawl_result}")
            continue
